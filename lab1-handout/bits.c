...
int sign(int x) {
  int sign = x>>31 & 1;
  int isZero = !x;
  int not_isZero = ~isZero + 1;
  int opp_sign = ~sign + 1;
  return not_isZero + opp_sign + !sign;
}
/*
...
int logicalNeg(int x) {
    int neg_num = x | (~x + 1);
    int shift = neg_num >> 31;
    int shift_plus1 = shift + 1;
  return shift_plus1;
}
/*
...
int isLessOrEqual(int x, int y) {
  int right_shift_x = x>>31;
  int sign = (!(right_shift_x))^(!(y>>31));
  int diff_sign_test = sign & (right_shift_x);
  int same_sign_test = (!sign) & !((y+(~x+1))>>31);
  return diff_sign_test | same_sign_test;
}
/* 
 ...
 */
int absVal(int x) {
  int condition = ~((x>>31)+1)+1;
  int opp_x = ~x+1;
  return (condition & x) | (~condition & opp_x);
}
/*
...
int isPower2(int x) {
  int min_1 = ~1 + 1;
  int x_min_1 = x + min_1;
  int max_neg_test = (x>>31)+1;
  int condition = (!(x_min_1 & x)) & max_neg_test & !!x;
  return (condition & 1) | (~condition & 0);
}
